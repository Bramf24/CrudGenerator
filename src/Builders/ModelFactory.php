    ...
    private function generateDefinitions(){
        ...
        foreach($this->modelFields as $name => $data){
            ...
            $output[] = match($data['type']){
                ...
                'character' => '            "'.$name.'" => $this->faker->text('.($data['maxlength'] ?? 1).'),',
                'character varying' => '            "'.$name.'" => $this->faker->text('.($data['maxlength'] ?? 1).'),',
                ...
                'smallint' => '            "'.$name.'" => $this->faker->numberBetween(1,'.($data['maxlength'] ?? 10).'),',
                'text' => '            "'.$name.'" => $this->faker->text('.($data['maxlength'] ?? 1).'),',
                'uuid' => '            "'.$name.'" => $this->faker->text('.($data['maxlength'] ?? 1).'),',
                'string' => '            "'.$name.'" => $this->faker->text('.($data['maxlength'] ?? 1).'),',
